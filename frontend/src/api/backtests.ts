...
export async function getBacktestTradesCsv(runId: number): Promise<BacktestTrade[]> {
  ...
  return rows.map((row, index) => ({
    ...
    radar_tier: row.radar_tier ? Number(row.radar_tier) : null,
    radar_profile: row.radar_profile === "pro1" || row.radar_profile === "pro2" || row.radar_profile === "pro3" ? row.radar_profile : null,
    radar_cycle_capital: row.radar_cycle_capital || null,
    source: row.source,
    created_at: "",
    updated_at: "",
  }));
