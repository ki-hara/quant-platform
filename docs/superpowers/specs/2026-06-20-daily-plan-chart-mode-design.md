...
1. Calculate the next trading session's LOC buy limit price and quantity from the last completed session.
2. Display the selected investment symbol's price chart with volume, LOC, and trade markers.
...
- LOC buy limit price uses the previous completed session's close.
- LOC quantity uses Capital, not Cash, divided by the confirmed mode's split count.
...
- The dashboard uses one integrated layout with the live mode, recommendation, LOC plan, investment chart, and QQQ RSI panel.
...
Refactor Dynamic Wave LOC calculation into one pure strategy operation used by live planning and backtesting. The operation accepts the previous close, confirmed/effective mode, Capital, Cash, open positions, fees, and strategy settings.
...
- Calculates the next actual market session's LOC limit price and quantity.
- Returns explicit availability and blocking reasons.

`ChartService`

- Returns ordered investment-symbol OHLCV data for the selected range.
- Adds the current LOC line and persisted live trade markers.
...
```text
LOC limit price = previous close * (1 + mode buy threshold / 100)
Per-order allocation = Capital / mode split count
Quantity = floor(Per-order allocation / LOC limit price)
Estimated fee = LOC limit price * Quantity * fee rate / 100
Estimated required cash = LOC limit price * Quantity + Estimated fee
```
...
The LOC plan is a limit-on-close instruction, so slippage is not added to its displayed
...
The backtest calculates the LOC limit from the preceding session before evaluating the current session. A buy fills only when the current session's close is at or below that limit, and the unadjusted fill price is the current close. The current high, low, and open do not trigger an LOC fill. The current close must not be used to create its own threshold.
...
Chart responses use typed DTOs for candlesticks, volume, RSI points, horizontal guides, LOC, and trade/mode markers. The API returns numeric values as decimal strings where precision matters.
...
4. Today's LOC limit price, quantity, formula inputs, availability, and reason.
5. Investment-symbol daily candlestick chart with volume, LOC line, and buy/sell markers.
...
- Fewer than 16 completed weekly closes: recommendation unavailable; confirmed mode remains unchanged. RSI(14) needs 15 closes for one value, and the mode rule needs two adjacent RSI values.
- Missing previous investment close: LOC plan unavailable.
...
### 11.3 LOC and Service Tests

- Exact LOC price and quantity formula.
...
- LOC inputs and blocking reasons are visible in Korean.
- Candlestick, volume, LOC, trade markers, and RSI panel render nonblank.
